...
2. YOLOv8 Pose detects the person's body position and keypoints.
3. The fall detection model analyses the input.
4. The application displays the result and confidence.

### For Videos

1. The user uploads a video and clicks **Run Analysis**.
2. Selected frames are sampled from the video.
3. YOLOv8 Pose analyses the person's posture in each sampled frame.
4. The system determines whether a potential fall is detected.
5. If a fall is found, the application displays the relevant fall frame and provides an alert.
6. If no fall is detected, the application clearly displays **No Fall Detected**.
